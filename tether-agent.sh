#!/bin/bash
# Tether agent — keeps audio in/out on the highest-priority device that is currently connected.
# Config: ~/.config/tether.json  ->  {"input": ["Name", ...], "output": ["Name", ...]}
...
pick() {
  ...
  wanted=$(jq -er --arg t "$type" '.[$t][]? | select(type == "string")' "$CONF" 2>&1) || {
    echo "tether: cannot read $type priorities from $CONF: $wanted" >&2
    return
  }
